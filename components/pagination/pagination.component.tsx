				...
				{pagination.pages.map((item, index: number) => {
					const sp1 =
						pagination.LEFT_PAGE && index === 0 ? (
							<span className={`pagination-list__item pagination-list_dots`}>...</span>
						) : undefined;
					const sp2 =
					...
					const li = (
						<li
							className={`pagination-list__item ${page === item ? 'pagination__item_active' : ''
								}`}
							key={item + 'pagination'}
						>
							{sp2}
							<button
								className={`pagination__btn ${page === item ? 'pagination__btn_active' : ''
									}`}
								data-page={item}
								onClick={onChangePagination}
							>
